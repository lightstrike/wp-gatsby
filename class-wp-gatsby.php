...
if ( ! class_exists( 'WP_Gatsby' ) ) {
	...
	class WP_Gatsby {
		...
		const VERSION        = '0.2.0';
		const REST_NAMESPACE = 'gatsby/v1';

		/**
		 * Initialization function that includes external files and fires WP hooks.
		 */
		...
		/**
		 * Include admin class that defines how WP admin users will configure the plugin.
		 */
		private static function includes() {
			include_once dirname( __FILE__ ) . '/includes/admin/classes/class-wp-gatsby-admin.php';
		}

		/**
		 * Various hooks to fire if WP Gatsby options are configured.
		 */
		private static function hooks() {
			$options = get_option( 'gatsby_options', array() );
			if ( 1 === $options['preview']['activated'] ) {
			...
			if ( 1 === $options['netlify']['auto_publish'] ) {
				self::set_netlify_auto_publish();
			}
		}

		/**
		 * Add a post updates endpoint to the WP REST API.
		 */
		public static function add_updates_rest_api_endpoint() {
			 ...
			 *
			 * @since  0.2.0
			 * @param  string $request WP request object.
			...
			function get_latest_updates( $request ) {
				...
				/**
				 * Run through posts array to remove duplicates, keeping only the latest update.
				 * Adapted from: https://stackoverflow.com/a/7872079 .
				 *
				 * @param  array $posts all post updates.
				 * @return array content updates data
				 */
        ...
        */
		public static function trigger_gatsby_refresh( $build_hook, $token, $cache ) {
			...
			return self::trigger_build( $build_hook, $args, $cache );
		}
        ...
        */
		public static function trigger_netlify_deploy( $build_hook, $cache ) {
			return self::trigger_build( $build_hook, array(), $cache );
		}
